...
BitTx loads & translates raw bitcoin transaction into TXO, ETXO, BOB or BSV2 JSON format, which can be:
...
BitTx can be used as a standalone tool for quickly making sense of bitcoin raw transactions
...
```javascript
 let options = {
                  network: "main" | "test"
                  format: "json" | "txo" | "bob" | "bsv" | "raw",
                  extended: true | false
                  getFormat: "arrayBuffer" | "json" | "text",
                  confirmations: true|false // To be done
                }

      async bittx.fromURL(url, options);
      url: the complete url including the the transaction id

      async bittx.fromTx(rawTx, options);
      async bittx.fromHash(txid, options);
      async bittx.loadExtended(txid, { format:'txo', network:"main"});

      async bittx.fromBSVDirect(txid, options);
      async bittx.fromBitails(txid, options);
      async bittx.fromElectrumX(txid, { format:"txo", network:"main"});
      async bittx.fromWoc(txid, {format:"woc", network:"main"});

// txid: the transaction id
// format: "woc" | "txo" | "bob" | "bsv" | "raw"
...
```
const bittx = require('bittx');
(async function() {
  let result = await bittx.fromTx("")
  console.log(result)
})();
```
...
The main job of bittx is to take a raw Bitcoin transaction and transform it into a structured format on top of which we can run all kinds of powerful query, processing, and filter.
...
## 2. bittx (Transaction Object)



### Level 1. Transaction

bittx represents a transaction as a JSON object, which follows the following high level format:
...
This is the top level view of what each transaction looks like when it's interpreted by bittx.
